...
public class BoardDaoImplTest {
    ...
    public void searchSelectPageTest() throws Exception{
        boardDao.deleteAll();
        for (int i=1; i<=20; i++){
            BoardDto boardDto = new BoardDto("no title"+i, "no content", "aaa"+i);
            boardDao.insert(boardDto);
        }

        SearchCondition sc = new SearchCondition(1, 10, "title2","T");
        List<BoardDto> list = boardDao.searchSelectPage(sc);
        System.out.println("list_T = " + list);
        assertTrue(list.size() == 2); // 1~20 title2, title20*/

        sc = new SearchCondition(1, 10, "aaa2","W");
        list = boardDao.searchSelectPage(sc);
        System.out.println("list_W = " + list);
        assertTrue(list.size() == 2); // 1~20 aaa2, aaa20
    }
    ...
    public void searchResultCntTest() throws Exception{
        ...
        SearchCondition sc = new SearchCondition(1, 10, "aaa2","W");
    ...
    public void insertTestDate() throws Exception{
        boardDao.deleteAll();
        for(int i=1; i<=220; i++){
            BoardDto boardDto = new BoardDto("title " + i, "no content", "aaa");
            boardDao.insert(boardDto);
        }
    }
